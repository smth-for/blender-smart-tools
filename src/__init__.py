...
from bpy.props import FloatProperty, PointerProperty, CollectionProperty, BoolProperty, StringProperty, IntProperty
...
class MeshToProcess(PropertyGroup):
    ...
    display_name: StringProperty(default="")  # Base case: empty list or with a single element
...
def create_low_collection(original_collection, color_07):
    """Create or get _low collection for decimated meshes."""
    low_name = f"{original_collection.name}_low"
    low_collection = bpy.data.collections.get(low_name)
    
    if not low_collection:
        low_collection = bpy.data.collections.new(low_name)
        bpy.context.scene.collection.children.link(low_collection)
    
    return low_collection

def create_object_copy(obj, low_collection):
    """Create a copy of the object in the low collection."""
    obj_low = obj.copy()
    obj_low.data = obj.data.copy()
    obj_low.name = f"{obj.name}_low"
    low_collection.objects.link(obj_low)
    return obj_low

def process_mesh(obj, obj_low):
    ...
    def execute(self, context):
        ...
        for obj, should_process in mesh_process_map.items():
            try:
                ...
                low_collection = create_low_collection(original_collection)
                obj_low = create_object_copy(obj, low_collection)
                
                if should_process:
                    success = process_mesh(obj, obj_low)
                    if success:
                        reduction = (1 - len(obj_low.data.vertices)/len(obj.data.vertices))*100
                        self.report({'INFO'}, f"Processed: {obj.name} - Reduction: {reduction:.1f}%")
                    else:
                        self.report({'WARNING'}, f"Error processing {obj.name}, copied without changes")
                else:
                    self.report({'INFO'}, f"Copied without processing: {obj.name}")
...
def find_common_prefix(names):
    ...
    if not names or len(names) == 1:  # Base case: empty list or with a single element
        return names if names else ""

    # Sort the names by length (the first one is the shortest)
    names.sort(key=len)
    first, second, *rest = names

    # Compare the first name with the second character by character
    for i, char in enumerate(first):
        if char!= second[i]:
            return first[:i]

    # If the first name is a prefix of the second, check the rest
...
def update_meshes_to_process(self, context):
    ...
    if scene.collection_to_process:
        # Cache of names and triangle counts
        mesh_data = []
        mesh_names = []
        ...
        # Calculate the common prefix only once
        ...
        for mesh_name, tri_count in mesh_data:
...
def create_lowpoly_collection(original_collection):
    """Create or get a _low collection for decimated meshes."""
    low_name = f"{original_collection.name}_low"
    low_collection = bpy.data.collections.get(low_name)
...
def apply_planar_decimation(obj, angle=5.0):
    """Apply planar decimation with given angle threshold."""
    mod = obj.modifiers.new(name="Decimate", type='DECIMATE')
    mod.decimate_type = 'DISSOLVE'  # Change to planar decimation
    mod.angle_limit = angle * (3.14159 / 180.0)  # Convert degrees to radians
    mod.use_dissolve_boundaries = False  # Preserve boundaries

    bpy.ops.object.modifier_apply(modifier=mod.name)

def decimate_mesh_planar(obj, low_collection, angle_threshold=5.0):
    """Process a single mesh with planar decimation."""
    ...
    apply_planar_decimation(obj_low, angle_threshold)

    obj_low.select_set(False)
    return obj_low

def transfer_data_planar(source, target):
...
class MESH_OT_batch_decimate_planar(Operator):
    ...
    def execute(self, context):
        ...
        for obj in selected_meshes:
            try:
                # Get or create low collection
                original_collection = obj.users_collection[0]
                low_collection = create_lowpoly_collection(original_collection)

                # Process mesh
                obj_low = decimate_mesh_planar(obj, low_collection, context.scene.planar_angle)

                # Transfer data from original to decimated mesh
                transfer_data_planar(obj, obj_low)  # Aggiunto qui

                self.report({'INFO'}, f"Processed: {obj.name} - Final faces: {len(obj_low.data.polygons)}")
...
def create_low_collection(original_collection):
    """Create or get a _low collection for decimated meshes."""
    low_name = f"{original_collection.name}_low"
    low_collection = bpy.data.collections.get(low_name)
...
def transfer_data(source, target):
    """Transfer normals and UVs from source to target."""
    # Transfer normals
    mod = target.modifiers.new(name="NormalTransferl", type='DATA_TRANSFER')
    mod.object = source
    mod.use_loop_data = True
    mod.data_types_loops = {'CUSTOM_NORMAL'}
    mod.loop_mapping = 'POLYINTERP_NEAREST'
...
def decimate_mesh(obj, low_collection, batch_decimate_ratio=0.5):
    ...
    def execute(self, context):
        ...
        for obj in selected_meshes:
            try:
                ...
                # Process mesh
                obj_low = decimate_mesh(obj, low_collection, context.scene.decimate_ratio)
                
                self.report({'INFO'}, f"Processed: {obj.name} - Reduction: {(1 - len(obj_low.data.vertices)/len(obj.data.vertices))*100:.1f}%")
